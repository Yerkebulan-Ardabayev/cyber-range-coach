...
создаётся один раз и разрешает диапазон адресов WSL `172.16.0.0/12`, а не один адрес.
Relay при этом слушает только адрес Windows на адаптере WSL, поэтому из Wi-Fi
его порты не открыты:
