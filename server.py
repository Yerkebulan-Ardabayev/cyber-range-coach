...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/range/health":
            ...
            return
        if path == "/api/today":
            lab = load_content("labs", "lab-1-2-service-enumeration")
            first_step = lab["steps"][0]
            with database() as connection:
            ...
            self.send_json(200, {"track": lab["track_id"], "lab": lab["id"], "step": first_step["id"], "assistance_level": level[0] if level else 4, "review_due": [{"skill_id": row[0], "due_date": row[1], "reason": row[2]} for row in review]})
            return
    ...
    def do_POST(self):
        ...
        with database() as connection:
            ...
            if action == "output":
                ...
                self.send_json(200, {"facts": facts, "unparsed": unparsed, "parser": parser_name})
                ...
                if choice not in options:
                    ...
                    return
                correct = choice == options[0]
                connection.execute("UPDATE attempt SET decision_choice = ? WHERE id = ?", (choice, attempt_id))
                why = "Этот вариант продолжает работу с подтверждённым сервисом." if correct else "Этот вариант уводит от подтверждённых фактов: перечисление ещё не заменяет оценку риска или выход за границу."
                self.send_json(200, {"correct": correct, "why": why, "next_step": "debrief" if correct else "analyse"})
            elif action == "debrief":
