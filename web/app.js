...
let parsedFacts = null, mentorFeedback = null, decisionFeedback = null, playbookMode = 'guided', playbookPhase = 0, browserStepIndex = 0;
...
const kindTitle = kind => ({think: 'Сначала сформулируй цель', execute: 'Выполни действие у себя', analyse: 'Разбери свою запись', decide: 'Выбери следующий шаг', debrief: 'Закрепи результат'})[kind] || kind;
const actionNoun = item => item && item.interaction ? (item.action_location === 'COACH' ? 'действие в Coach' : 'действие в Chrome') : 'команду';
...
function phaseBody(item) {
  if (item.kind === 'think') {
    const nextInteraction = lab.steps[phaseIndex + 1] && lab.steps[phaseIndex + 1].interaction;
    const nextNoun = nextInteraction ? 'действия' : 'команды';
    return `<p class="lead">${esc(item.prompt)}</p>${whyBox(item)}<label class="input-label" for="hypothesis">Моя гипотеза до ${nextNoun}</label><textarea id="hypothesis" placeholder="Например: хочу увидеть один наблюдаемый факт на своём учебном стенде. Не пиши название уязвимости, если вывода ещё нет."></textarea><p class="input-hint">Пиши наблюдаемый результат, который ожидаешь увидеть. После сохранения появится ровно один следующий шаг.</p><button id="save-hypothesis">Сохранить гипотезу и перейти к ${nextNoun}</button><div id="form-error"></div>`;
  }
  if (item.kind === 'execute') {
    if (item.interaction === 'browser-observation' || item.interaction === 'guided-observation') {
      const browserSteps = item.browser_steps || [];
      const currentBrowserStep = browserSteps[browserStepIndex];
      const action = currentBrowserStep ? `<section class="browser-steps"><p class="eyebrow">ДЕЙСТВИЕ ${browserStepIndex + 1} ИЗ ${browserSteps.length} · ${esc(item.action_location || 'CHROME')}</p><p>${esc(currentBrowserStep)}</p><button id="next-browser-step">Я сделал это, показать следующий шаг</button></section>` : `<details class="command-anatomy" open><summary>Что это значит и зачем</summary><ol>${(item.command_anatomy || []).map(part => `<li>${esc(part)}</li>`).join('')}</ol></details><label class="input-label" for="raw-output">${esc(item.observation_label || 'Моя запись наблюдения')}</label><textarea id="raw-output" spellcheck="false" placeholder="${esc(item.observation_placeholder || 'Вставь наблюдаемые факты по одному на строку')}"></textarea><p class="input-hint">${esc(item.observation_hint || 'Это не команда и не вывод Terminal. Запиши только наблюдаемые поля.')}</p><button id="parse-output">Сохранить запись и перейти к разбору</button><div id="form-error"></div>${help(item)}`;
      return `<p class="lead">${esc(item.prompt)}</p>${whyBox(item)}${action}`;
    }
...
function bindRunner(item) {
  const nextBrowserStep = document.querySelector('#next-browser-step');
  if (nextBrowserStep) nextBrowserStep.onclick = () => { browserStepIndex += 1; runner(); };
  ...
  if (save) save.onclick = async () => { const hypothesis = document.querySelector('#hypothesis').value.trim(); if (!hypothesis) { showError('Сначала напиши гипотезу: что именно следующее действие должно помочь тебе узнать.'); return; } try { await createAttempt(lab.steps[phaseIndex + 1], hypothesis); phaseIndex += 1; await runner(); } catch (error) { showError(error); } };
  const parse = document.querySelector('#parse-output');
  if (parse) parse.onclick = async () => { const raw = document.querySelector('#raw-output').value.trim(); if (!raw) { showError(item.interaction ? 'Сначала запиши наблюдаемые поля из Chrome или Coach.' : 'Вставь весь фактический вывод Terminal. Если есть ошибка или пустой результат, вставь сообщение об этом.'); return; } try { parsedFacts = await post('/api/attempt/' + attemptId + '/output', {raw}); await createAttempt(lab.steps[phaseIndex + 1], item.interaction ? 'Разобрать факты из сохранённой записи наблюдения.' : 'Разобрать факты из сохранённого вывода команды.'); phaseIndex += 1; await runner(); } catch (error) { showError(error); } };
  ...
  if (close) close.onclick = async () => { try { const result = await post('/api/attempt/' + attemptId + '/debrief', {verdict: 'completed'}); const learning = result.learning && result.learning.length ? ` Навык обновлён: уровень ${result.learning[0].level}, помощь L${result.learning[0].assistance_level}.` : ''; app.innerHTML = `<section class="page completion"><p class="eyebrow">ПОПЫТКА СОХРАНЕНА</p><h1>Лабораторная закрыта</h1><p>Field Note создана из твоей гипотезы, сохранённой записи и собственного заключения.${esc(learning)}</p><div class="completion-actions"><button id="next-lab">Открыть следующий учебный шаг</button><a class="text-link" href="#notes">Посмотреть Field Note</a></div></section>`; document.querySelector('#next-lab').onclick = () => { attemptId = null; labId = null; phaseIndex = 0; parsedFacts = null; mentorFeedback = null; decisionFeedback = null; location.hash = '#today'; }; } catch (error) { showError(error); } };
  bindHelp(item);
}
async function today() {
  ...
  const executeStep = current && current.steps.find(item => item.kind === 'execute');
  app.innerHTML = `<section class="page today-page"><p class="eyebrow">ОДНА ЯСНАЯ ТОЧКА ВХОДА</p><h1>Сегодня</h1><article class="today-card"><p class="eyebrow">${data.resume ? 'НЕЗАВЕРШЁННАЯ ПОПЫТКА' : 'СЛЕДУЮЩАЯ ЛАБОРАТОРНАЯ'}</p><h2>${esc(current ? current.objective : data.lab)}</h2><p><strong>Сейчас:</strong> ${esc(kindTitle(data.step))}. ${data.resume ? 'Сайт вернёт к тому же шагу и восстановит твои факты.' : 'Начни с цели, затем получишь ровно одно ' + esc(actionNoun(executeStep)) + '.'}</p><button id="go">${data.resume ? 'Продолжить с сохранённого места' : 'Начать лабораторную'}</button></article>${catalog}</section>`;
  document.querySelector('#go').onclick = () => { labId = data.lab; attemptId = data.resume ? data.resume.attempt_id : null; phaseIndex = 0; browserStepIndex = 0; parsedFacts = null; mentorFeedback = null; decisionFeedback = null; location.hash = '#lab-runner'; };
  document.querySelectorAll('[data-lab]').forEach(button => button.onclick = () => { labId = button.dataset.lab; attemptId = null; phaseIndex = 0; browserStepIndex = 0; parsedFacts = null; mentorFeedback = null; decisionFeedback = null; location.hash = '#lab-runner'; });
}
async function rangePage() {
  setActive('range'); const data = await api('/api/range/health');
  app.innerHTML = `<section class="page"><p class="eyebrow">ГОТОВНОСТЬ УЧЕБНОГО СТЕНДА</p><h1>Range: ${esc(data.verdict)}</h1><p class="range-host">Стенд: ${esc(data.host || 'не найден')}${data.discovery === 'sweep' ? ' (адрес найден заново поиском в своей подсети)' : ''}</p><p>${data.verdict === 'ONLINE' ? 'Оба учебных сервиса отвечают. Открой нужную лабораторную: она сама покажет, действие выполняется в Terminal, Chrome или Coach.' : 'Сайт не будет выдумывать факт: сначала восстанови стенд, затем вернись к указанному действию лабораторной.'}</p><div class="range-checks">${(data.checks || []).map(item => `<article class="range-check ${item.ok ? 'ok' : 'bad'}"><h3>${esc(item.name || ('Порт ' + item.port))}</h3><p>${esc(item.detail || '')}</p></article>`).join('')}</div><section class="diagnosis"><h2>Что проверить</h2>${(data.diagnosis || []).map(item => `<p class="${esc(item.level)}">${esc(item.message)}</p>`).join('')}</section><p class="context-rule">Эта проверка сверяет только два своих сервиса, WebGoat на 8080 и Juice Shop на 3000. Если стенд сменил адрес, Coach ищет его в своей подсети по этим двум портам, не чаще раза в минуту. Сканирование и атаки выполняет только человек на своей разрешённой цели.</p></section>`;
}
async function notes() {
  setActive('notes'); const items = await api('/api/notes');
  app.innerHTML = `<section class="page"><h1>Field Notes</h1><p>Твой журнал: что ожидал, какое действие или команду выполнил, какую запись сохранил и какой вывод сделал.</p>${items.map(item => `<article class="note"><h2>${esc(item.date)} · ${esc(item.goal)}</h2><dl><dt>Гипотеза</dt><dd>${esc(item.hypothesis)}</dd><dt>Действие или команда</dt><dd><code>${esc(item.commands)}</code></dd><dt>Сохранённая запись</dt><dd><pre>${esc(item.results)}</pre></dd><dt>Твоё заключение</dt><dd>${esc(item.conclusion)}</dd></dl><label for="note-${item.id}">Личный итог одним абзацем</label><textarea id="note-${item.id}" data-note="${item.id}" placeholder="Что я сделаю иначе в следующий раз">${esc(item.manual_paragraph)}</textarea><button data-save-note="${item.id}">Сохранить мой итог</button></article>`).join('') || '<div class="empty-state"><strong>Пока нет закрытых попыток</strong><span>Одна закрытая лабораторная создаст здесь читаемую Field Note.</span></div>'}</section>`;
